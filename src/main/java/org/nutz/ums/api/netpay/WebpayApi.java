...
import org.nutz.ums.bean.webpay.req.*;
import org.nutz.ums.bean.webpay.resp.*;
...
public class WebpayApi {
    ...
    public static String pay(PayReq req) {
        ...
        try {
            ...
            } else if (Strings.isBlank(req.getContent())) {
            ...
            } else if (Lang.isEmpty(req.getContent())) {
                throw new NullPointerException("content为空");
            } else {
                return Comm.NETPAY_WEBPAY_PAY_ACTION + "?authorization=" + req.getAuthorization()
                        + "&appId=" + req.getAppId() + "&timestamp=" + req.getTimestamp()
                        + "&nonce=" + req.getNonce() + "&content=" + Util.Url.encode(req.getContent())
     ...
     * @return
     */
    public static JspayResp jspay(JspayReq req) {
        ...
        try {
            if (Strings.isBlank(req.getAccessToken())) {
                ...
                throw new NullPointerException("merOrderId为空");
            } else if (Util.isAllBlank(req.getUserId(), req.getCode())) {
